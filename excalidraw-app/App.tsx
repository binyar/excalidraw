...
> => {
  ...
  const workspaceFileId = searchParams.get("workspace");
  if (workspaceFileId) {
    try {
      ...
      const data = await loadFromBlob(await response.blob(), null, null);
      return {
        scene: { ...data, scrollToContent: true },
...
const ExcalidrawWrapper = () => {
  ...
  const workspaceFileId = new URLSearchParams(window.location.search).get(
    "workspace",
  );
    ...
    const unloadHandler = (event: BeforeUnloadEvent) => {
      ...
      if (workspaceFileId && pendingWorkspaceContent.current) {
        ...
        fetch(`/api/workspace/files/${workspaceFileId}/content`, {
          ...
          console.error("离开页面前保存工作区文件失败", error);
        });
      }
  ...
  const flushWorkspaceSave = useCallback(
    async ({ force = false, notify = false } = {}) => {
      ...
      const contentToSave =
        pendingWorkspaceContent.current ||
        (force && excalidrawAPI
          ? serializeAsJSON(
      ...
      try {
        ...
        if (pendingWorkspaceContent.current === contentToSave) {
  ...
  useEffect(() => {
    const saveShortcutHandler = (event: KeyboardEvent) => {
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "s") {
        ...
        flushWorkspaceSave({ force: true, notify: true }).catch((error) => {
          console.error("手动保存工作区文件失败", error);
        });
      }
    };
    window.addEventListener("keydown", saveShortcutHandler, true);
    return () =>
      window.removeEventListener("keydown", saveShortcutHandler, true);
  }, [flushWorkspaceSave]);
  ...
  const onChange = (
    ...
    if (workspaceFileId) {
      pendingWorkspaceContent.current = serializeAsJSON(
        elements,
        appState,
        files,
        "local",
      ...
      workspaceSaveTimer.current = window.setTimeout(() => {
        flushWorkspaceSave().catch((error) => {
          console.error("工作区文件自动保存失败", error);
        });
      }, 700);
    }
    ...
    if (!workspaceFileId && !LocalData.isSavePaused()) {
