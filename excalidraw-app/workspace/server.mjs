...
const serializeFileWrite = async (storageName, write) => {
  const previous = fileWriteQueues.get(storageName) || Promise.resolve();
  const current = previous.catch(() => {}).then(write);
  fileWriteQueues.set(storageName, current);
  try {
    return await current;
  } finally {
    if (fileWriteQueues.get(storageName) === current) {
      fileWriteQueues.delete(storageName);
    }
  }
};

const replaceFileAtomically = async (targetPath, content) => {
...
const listItems = (url) => {
  ...
  const sort = ["name", "created_at", "updated_at", "size"].includes(
...
export const handleWorkspaceRequest = async (req, res) => {
  ...
  try {
    ...
    if (resource === "folders" && id && req.method === "PATCH") {
      ...
      ).run(normalizeName(body.name, row.name), now(), id);
      ...
      if (url.searchParams.get("permanent") === "true") {
        ...
        await Promise.all(
          stored.map(({ storage_name }) =>
            rm(storagePath(storage_name), { force: true }),
          ),
        );
      } else {
        db.prepare(
    ...
    if (req.method === "POST" && resource === "files" && !id) {
      ...
      db.prepare(
        ...
        fileId,
        normalizeDrawingName(body.name),
        body.folderId || null,
        storageName,
    ...
    if (
      ...
      if (url.searchParams.get("preview") !== "true") {
        db.prepare("UPDATE files SET last_opened_at = ? WHERE id = ?").run(
          now(),
          id,
        );
      }
      ...
      res.writeHead(200, {
        ...
        "x-workspace-file-name": encodeURIComponent(row.name),
      });
      res.end(content);
      return true;
    }
    if (
      ...
      try {
        ...
        throw Object.assign(new Error("画板内容不是有效的 JSON"), {
          status: 400,
        });
      }
      if (!isDrawing(parsed))
        throw Object.assign(new Error("画板内容格式不正确"), { status: 400 });
      await serializeFileWrite(row.storage_name, async () => {
        await replaceFileAtomically(storagePath(row.storage_name), body);
        db.prepare(
    ...
    if (resource === "files" && id && req.method === "PATCH") {
      ...
      db.prepare(
        ...
        body.isFavorite === undefined
          ? row.is_favorite
          : Number(parseBoolean(body.isFavorite)),
        now(),
        id,
      );
      sendJson(res, 200, mapFile(getFileRow(id)));
    ...
    if (
      ...
      db.prepare(
        "UPDATE files SET is_deleted = 0, updated_at = ? WHERE id = ?",
      ).run(now(), id);
      sendJson(res, 200, mapFile(getFileRow(id)));
      return true;
    }
